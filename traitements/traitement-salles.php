...
function createSalle($nom, $localisation){
    ...
    $req = $bdd->prepare("INSERT INTO t_salle(nom, localisation) VALUES(:nom, :localisation)");
...
function updateSalle($id, $nom, $localisation){
    ...
    $req = $bdd->prepare("UPDATE t_salle SET nom = :nom,  localisation = :localisation WHERE id = :id");
    $req->bindParam(':id', $id);
    $req->bindParam(':nom', $nom);
    $req->bindParam(':localisation', $localisation);
...
function deleteSalle($id){
    include('database.php');
    $req = $bdd->prepare("DELETE FROM t_salle WHERE id = :id");
